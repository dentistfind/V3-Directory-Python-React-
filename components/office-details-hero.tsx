...
export default function OfficeDetailsHero({ officeData }: {officeData: OfficeData}){
    const { name, logoUrl, isSponsored, rating, reviewCount, address, videoUrl: { videoSrc, videoThumbnail} } = officeData

    return(
        <div className="bg-white text-black">
            <p className="text-xs flex items-center gap-1 p-5"><Link href={HOME_PAGE_URL} className="text-gray-400 flex items-center gap-1 cursor-pointer">DentistFind <FaChevronRight className="text-xs" /></Link> {name}</p>
            ...
            <div className="bg-theme-light px-20 py-5 flex justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1 font-semibold text-sm"><CiLocationOn /> {address}</div>
